...
(describe "Testing Main"
  ...
  (it "continue a previous game"
    (with-redefs [ui/print-resume-game (stub :print-resume-game)]
      (let [input-id "123"
            game {:game-id  2
                  :player-1 {:kind :human :token "X"} :player-2 {:kind :human :token "X"}}
  ...
  (it "possible to continue a game"
    (with-redefs [board/game-over? (stub :game-over? {:return false})]
      (let [game {:game-id 2 :player-1 {:kind :human :token "X"} :player-2 {:kind :human :token "X"} :size :3x3 :moves []}]
  ...
  (it "continue a last game"
    (with-redefs [ui/continue-last-game? (stub :continue-last-game? {:return true})
      ...
      (let [last-game {:game-id 2 :player-1 {:kind :human :token "X"} :player-2 {:kind :human :token "X"} :size :3x3 :moves []}]
