...
(describe "Testing Main"
  ...
  (it "starts a new game correctly"
    (with-redefs [ui/get-game-board (constantly :3x3)
      ...
      (let [game (sut/start-new-game 2)]
        (should= [{:game-id 2 :board [1 2 3 4 5 6 7 8 9] :player-1 {:kind :human :token "X"} :player-2 {:kind :human :token "X"}, :size :3x3, :moves []} 2]
                 game))))

  (it "continue a previous game"
    (with-redefs [ui/print-previous-moves-game (stub :print-previous-moves-game)]
      (let [input-id "123"
            game {:game-id 2 :board [1 2 3 4 5 6 7 8 9] :player-1 {:kind :human :token "X"} :player-2 {:kind :human :token "X"}}
  ...
  (it "possible to continue a game"
    (with-redefs [board/game-over? (stub :game-over? {:return false})]
      (let [game {:game-id 2 :board [1 2 3 4 5 6 7 8 9] :player-1 {:kind :human :token "X"} :player-2 {:kind :human :token "X"}}]
  ...
  (it "continue a last game"
    (with-redefs [ui/continue-last-game? (stub :continue-last-game? {:return true})
      ...
      (let [last-game {:game-id 2 :board [1 2 3 4 5 6 7 8 9] :player-1 {:kind :human :token "X"} :player-2 {:kind :human :token "X"}}]
  ...
  (it "gets a game by id"
    (with-redefs [data/game-history-by-id (stub :game-history-by-id
      ...
      (let [id 2
            expected-game {:game-id 2 :board [1 2 3 4 5 6 7 8 9] :player-1 {:kind :human :token "X"} :player-2 {:kind :human :token "X"}}
  ...
  (it "starts a new game if no game can be continued"
    (with-redefs [data/get-next-game-id (stub :get-next-game-id {:return 4})
                  ...
                  sut/start-new-game (stub :start-new-game {:return [{:game-id 4 :board [1 2 3 4 5 6 7 8 9] :player-1 {:kind :human :token "X"} :player-2 {:kind :human :token "X"}} 4]})]
      (let [input-id 2
            expected-game {:game-id 4 :board [1 2 3 4 5 6 7 8 9] :player-1 {:kind :human :token "X"} :player-2 {:kind :human :token "X"}}
            actual-game (sut/continue-game? input-id)]
        (should= expected-game (first actual-game)))))

  (it "continues a requested game if possible"
    (with-redefs [sut/game-by-id (stub :game-by-id {:return {:game-id 2 :board [1 2 3 4 5 6 7 8 9] :player-1 {:kind :human :token "X"} :player-2 {:kind :human :token "X"}}})
                  sut/possible-to-continue? (stub :possible-to-continue? {:return true})
                  sut/continue-previous-game (stub :continue-previous-game {:return [{:game-id 2 :board [1 2 3 4 5 6 7 8 9] :player-1 {:kind :human :token "X"} :player-2 {:kind :human :token "X"}} 2]})]
      (let [input-id 2
            expected-game {:game-id 2 :board [1 2 3 4 5 6 7 8 9] :player-1 {:kind :human :token "X"} :player-2 {:kind :human :token "X"}}
            actual-game (sut/continue-game? input-id)]
        (should= expected-game (first actual-game)))))
  ...
  (it "continues the last game if possible"
    (with-redefs [data/get-last-game (stub :get-last-game {:return {:game-id 3 :board [1 2 3 4 5 6 7 8 9] :player-1 {:kind :human :token "X"} :player-2 {:kind :human :token "X"}}})
                  data/last-game-id (stub :last-game-id {:return 3})
                  sut/possible-to-continue? (stub :possible-to-continue? {:return false})
                  sut/continue-last-game? (stub :continue-last-game? {:return true})
                  sut/continue-previous-game (stub :continue-previous-game {:return [{:game-id 3 :board [1 2 3 4 5 6 7 8 9] :player-1 {:kind :human :token "X"} :player-2 {:kind :human :token "X"}} 3]})]
      (let [input-id 2
            expected-game {:game-id 3 :board [1 2 3 4 5 6 7 8 9] :player-1 {:kind :human :token "X"} :player-2 {:kind :human :token "X"}}
            actual-game (sut/continue-game? input-id)]
        (should= expected-game (first actual-game)))))

  (it "tests the -main function"
    (with-redefs [sut/continue-game? (stub :continue-game? {:return [{:game-id 1 :board [1 2 3 4 5 6 7 8 9] :player-1 {:kind :human :token "X"} :player-2 {:kind :human :token "X"} :moves []} 1]})
                  ui/print-id-and-empty-board (stub :print-id-and-empty-board)
                  gm/play-round (stub :play-round {:return {:game-id 1 :board [1 2 3 4 5 6 7 8 9] :player-1 {:kind :human :token "X"} :player-2 {:kind :human :token "X"} :moves []}})
